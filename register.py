...
try:
    ...
    sql = 'INSERT INTO register(first_name,last_name,email_id,phone,username,password,salt) VALUES("%s", "%s", "%s","%s","%s","%s","%s")' %(First_Name,Last_Name,Email,Phone,Username,new_password,salt)
    #print(sql)
    c.execute(sql)
    conn.commit()
    
    sql1 = 'INSERT INTO payment_page(username,customer_profile_id)VALUES("%s","%d")' %(Username,customer_profile_id)
    #print(sql1)
    c.execute(sql1)
    conn.commit()
...
try:
    ...
    pass
#print("Content-type:text/html\n\n")
redirectURL = "http://pyb78.specind.net/login.html"
print("<html>")
print("<head>")
